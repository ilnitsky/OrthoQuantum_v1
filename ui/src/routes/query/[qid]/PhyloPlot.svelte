<script lang="ts">
	...
	export let leafCound: number;

	let firstLoad: number|undefined = addAlert(
		"Loading phylomap, please wait",
		undefined,
		undefined,
		false,
		false
	...
	async function redraw_phyd3(url: string, taskid: string, leafCount: number) {
		...
		interactionQueue = interactionQueue
			...
			.then(() => {
				...
				if (firstLoad){
					updateAlert(firstLoad, (a)=>{
						a.isOpen=false;
						a.isPersistant=false;
					})
					firstLoad = undefined;
				}
			});
		...
		resizeTimer = setTimeout(() => {
			interactionQueue = interactionQueue.then(() => {
				if (isInteractive && phyd3_rendered && renderedWidth < elementWidth) {
					redraw_phyd3(url, taskid, leafCound);
				}
			});
		}, 1500);
	}
